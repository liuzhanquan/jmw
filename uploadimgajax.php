...
if (!isset($_REQUEST["userInfo"])){
	exit(json_encode(['message'=>'登录才能上传','status'=>400]));
//echo "<script>alert('登录后才能上传');window.close();</script>";
}else{
	$userInfo = str_replace(" ","+",$_REQUEST["userInfo"]);
	$userInfo = json_decode(authcode($userInfo,'DECODE'),true);
...
$filename = array();  
$filename[0]['name']=$_FILES['file']['name'];
$filename[0]['type']=$_FILES['file']['type'];
$filename[0]['tmp_name']=$_FILES['file']['tmp_name'];
$filename[0]['error']=$_FILES['file']['error'];
$filename[0]['size']=$_FILES['file']['size'];
if(empty($filename)) exit(json_encode(['message'=>'上传图片为空','status'=>400]));	
...
$isok = query("update zzcms_user set img='$bigimg' where id='$uid'");
if($isok){
	exit(json_encode(['message'=>'修改成功','status'=>200,'data'=>$bigimg]));	
}else{
	exit(json_encode(['message'=>'修改失败','status'=>400]));		
}
