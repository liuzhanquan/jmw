...
$config['flash_dir']='/uploadfiles/'.date("Y-m"); //上传flash文件地址 采用绝对地址 方便upload.php文件放在站内的任何位置 
...
$data=GetImageSize($file_host);//取得GIF、JPEG、PNG或SWF图片属性，返回数组，图形的宽度[0],图形的高度[1]，文件类型[2]
	if($data[2]!=4){//文件类型不为4，4为swf格式
    	switch ($data[2]) {
     	case 1 :
      	$sImg = imagecreatefromgif($file_host);
      	break;
     	case 2 :
      	$sImg = imagecreatefromjpeg($file_host);
      	break;
     	case 3 :
      	$sImg = imagecreatefrompng($file_host);
     	 break;
     	case 6 :
      	$sImg = imagecreatefromwbmp($file_host);
      	break;
     	default :
	 	mkhtml($fn,"","不支持的文件类型，无法生成缩略图");
    	}
		//生成小图
		if ($data[1]>$data[0])
		{
