...
$rs = query("select classid,parentid,classname from zzcms_zsclass where ".$where." order by xuhao"); 
if ($rs){//当不出错时

while($row = fetch_array($rs)){
	
	$str=$str. "<a href=?b=".$row['parentid']."&pid=".$row['classid'].">";  
	if ($row["parentid"]==$b && $row['classid'] == $pid) {
	$str=$str."<b>".$row["classname"]."</b>";
	}else{
	$str=$str.$row["classname"];
	}
...
<form name="myform" id="myform" method="post" action="" onSubmit="return anyCheck(this.form)">
      ...
      <div class="border2"> 
        ...
        <input name="tablenametow" type="hidden"  value="zzcms_dlmessage"> </div>
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="1">
    <tr class="trtitle"> 
      ...
      <td width="8%">电话</td>
      <td width="10%">发布人</td>
      <td width="5%" align="center">接收者</td>
      <td width="10%">发布时间</td>
...
?>
    <tr class="trcontent"> 
	 ...
	 <td><?php echo $row["id"]?></td>
      <td><?php echo dl_level_sel($row["id"],1);?></td>
	...
	echo '非留言';
      ...
      <td align="center" class="docolor"> 
		<a href="dl.php?do=modify&id=<?php echo $row["id"]?>&page=<?php echo $page ?>">修改</a> |     
		<a href="dlmessage.php?did=<?php echo $row["id"]?>&page=<?php echo $page ?>">加盟申请</a> 
		<br/>
		<a href="dlgg.php?did=<?php echo $row["id"]?>&page=<?php echo $page ?>">广告</a>      
