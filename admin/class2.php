...
if ($action=="isshow") {
	...
	empty($state) ? $state = 1 : $state = 0;
	
	query("update `zzcms_zsclass` set $type='$state' where classid='$id'");
...
if ($action=="adclass") {
	$id=isset($_REQUEST['id'])?$_REQUEST['id']:'';
	//是否存在该广告  存在 得到id  不存在 0 
	$state=isset($_REQUEST['state'])?$_REQUEST['state']:'';
	$type=isset($_REQUEST['type'])?$_REQUEST['type']:'';
	if(empty($state)){
		$zs = query_arr("select classname from zzcms_zsclass where classid='$id'");
		$ad = query_arr("select classname from zzcms_adclass where classid='$type'");
		$classname = $zs[0]['classname'];
		$parentid = $ad[0]['classname'];
		
		query("insert `zzcms_adclass` (classname,parentid,xuhao,dl_classid) value('$classname','$parentid',0,'$id')");
	}else{
		
		query("delete from `zzcms_adclass` where classid='$state'");
...
<form name="form1" method="post" action="?action=px">
  <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
    <tr class="trtitle"> 
      ...
      <td width="7%"  >ID</td>
      <td width="5%"  >拼音</td>
      <td width="8%" align="center" >显示/隐藏分类 </td>
      <td width="8%" align="center" >首页导航显示 </td>
      <td width="8%" align="center" >为你推荐显示 </td>
      <td width="8%" align="center" >热门分类显示 </td>
      <td width="15%" align="center" >所用模板文件</td>
      <td width="15%"  >排序</td>
      <td width="20%"  >操作</td>
    </tr>
    ...
    <tr class="bgcolor1"> 
      ...
      <td align="center"><a href="class2.php?action=isshow&type=isshow&id=<?php echo $row["classid"];?>&state=<?php echo $row["isshow"];?>">是否显示</a> [ 
		<?php if ($row["isshow"]==1) { echo "显示";} else{ echo "<font color=red>不显</font>";}?> ] 
	  </td>
	  <td align="center"><a href="class2.php?action=isshow&type=showindex&id=<?php echo $row["classid"];?>&state=<?php echo $row["showindex"];?>">是否显示</a>  [ 
		<?php if ($row["showindex"]==1) { echo "显示";} else{ echo "<font color=red>不显</font>";}?> ] 
	  </td>
	  <td align="center"><a href="class2.php?action=adclass&type=35&id=<?php echo $row["classid"];?>&state=<?php echo getadstatus($row['classid'],35);?>">是否显示</a>  [ 
		<?php if (getadstatus($row['classid'],35)!=0) { echo "显示";} else{ echo "<font color=red>不显</font>";}?> ] 
	  </td>
	  <td align="center"><a href="class2.php?action=adclass&type=36&id=<?php echo $row["classid"];?>&state=<?php echo getadstatus($row['classid'],36);?>">是否显示</a>  [ 
		<?php if (getadstatus($row['classid'],36)!=0) { echo "显示";} else{ echo "<font color=red>不显</font>";}?> ] 
	  </td>
      <td >
	  <?php
    ...
    <tr class="trcontent">  
      ...
      <td align="center"><a href="class2.php?action=isshow&type=isshow&id=<?php echo $rown["classid"];?>&state=<?php echo $rown["isshow"];?>">是否显示</a> [ 
		<?php if ($rown["isshow"]==1) { echo "显示";} else{ echo "<font color=red>不显</font>";}?> ] 
	  </td>
	  <td align="center"><a href="class2.php?action=isshow&type=showindex&id=<?php echo $rown["classid"];?>&state=<?php echo $rown["showindex"];?>">是否显示</a> [ 
		<?php if ($rown["showindex"]==1) { echo "显示";} else{ echo "<font color=red>不显</font>";}?> ] 
	  </td>
	  <td align="center"><a href="class2.php?action=adclass&type=35&id=<?php echo $rown["classid"];?>&state=<?php echo getadstatus($rown['classid'],35);?>">是否显示</a> [ 
		<?php if (getadstatus($rown['classid'],35)!=0) { echo "显示";} else{ echo "<font color=red>不显</font>";}?> ] 
	  </td>
	  <td align="center"><a href="class2.php?action=adclass&type=36&id=<?php echo $rown["classid"];?>&state=<?php echo getadstatus($rown['classid'],36);?>">是否显示</a>  [ 
		<?php if (getadstatus($rown['classid'],36)!=0) { echo "显示";} else{ echo "<font color=red>不显</font>";}?> ] 
	  </td>
      <td>&nbsp;</td>
...
<form name="form1" method="post" action="?dowhat=modifybigclass" onSubmit="return CheckForm();">
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    ...
    <tr>
      <td align="right" class="border">图标： </td>
		<td class="border"> 
			<input name="img" type="hidden" id="img" >
			  ...
			  <tr align="center" bgcolor="#FFFFFF"> 
				<td id="showimg" onClick="openwindow('/uploadimg_form.php?noshuiyin=1',400,300)">
					<?php if( empty($row['img']) ){ 
						...
						echo '<img src='.$row['img'].' width=120>';
					} ?>
				</td>
			  </tr>
			</table>
...
if ($description==""){$description=$classname;}
	...
	title='$title',keyword='$keyword',description='$description',img='$img' where  classid='" .$classid."'");
