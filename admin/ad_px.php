...
if (@$_REQUEST["action"]=="px" && !empty($_POST)){
checkadminisdo("adv");
$b = $_POST['b'];
$s = $_POST['s'];
if ($s==''){
...
?>
    <tr class="trcontent"> 
	  ...
	  <?php 
		...
		$dzStr = '';
		$dl_sql = query_arr('select cp from zzcms_dl where id="'.$row['dl_id'].'"');
		$zx_sql = query_arr('select title,bigclassid,smallclassid from zzcms_zx where id="'.$row['zx_id'].'"');
		if( !empty($dl_sql) ){
			$dzStr = $dzStr.'产品标题：'.dl_level_sel($row['dl_id'],1,$dl_sql[0]['cp']);
		}
		if( !empty($dl_sql) && !empty($zx_sql) ){
			$dzStr = $dzStr.'<br/>';
		}
		if( !empty($zx_sql) ){
			if( empty($zx_sql[0]['smallclassid']) ){
				$dzStr = $dzStr.'资讯标题：<a href="zx_manage.php?b='.$zx_sql[0]['bigclassid'].'">'.$zx_sql[0]['title'].'</a>';
			}else{
				$dzStr = $dzStr.'资讯标题：<a href="zx_manage.php?b='.$zx_sql[0]['bigclassid'].'&s='.$zx_sql[0]['smallclassid'].'">'.$zx_sql[0]['title'].'</a>';
			}
		}
	?>
	 <td width="156"><?php echo $dzStr;?></td>
