...
if(!empty($_POST['id'])){
    ...
    $id=$_POST['id'][$i];
	checkid($id);
	$sql="select passed from zzcms_footprint where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
	if ($row['passed']=='0'){
	query("update zzcms_footprint set passed=1 where id ='$id'");
    }else{
	query("update zzcms_footprint set passed=0 where id ='$id'");
	}
	}	
}else{
...
if ($do=="modify"){
	if(!empty($_GET['id'])){
		...
		$val=$_GET['value'];
		checkid($id);
		query("update zzcms_footprint set state=$val where id ='$id'");
	}else{
		echo "<script lanage='javascript'>alert('操作失败！至少要选中一条信息。');history.back()</script>";
	}
...
<body>
<div class="admintitle">用户足迹查看</div>
...
$sql="select count(*) as total from zzcms_footprint where id<>0 ";
$rs =query($sql.$sql2); 
$row = fetch_array($rs);
$totlenum = $row['total'];
$totlepage=ceil($totlenum/$page_size);

$sql="select * from zzcms_footprint where id<>0 ";
$sql=$sql.$sql2;
$sql=$sql . " order by id desc limit $offset,$page_size";
//$sql=$sql." and id>=(select id from zzcms_footprint order by id limit $offset,1) order by id desc limit $page_size";
$rs = query($sql); 
if(!$totlenum){
echo "暂无信息";
}else{
