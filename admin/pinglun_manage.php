...
if ( $zid<>'0' && !empty($zid) ) {
	$sql2 = $sql2."and about ='".$zid."'";
}
if ( $uid<>'0' && !empty($uid) ) {
	$sql2 = $sql2."and user_id ='".$uid."'";
}
...
?>
    <tr class="trcontent"> 
      ...
      <td >
		<a href="<?php echo getpageurl("zx",$row["about"])?>" target="_blank"><?php echo $row["about"]?></a>
		<a href="dl_manage.php?id=<?php echo $row['about'];?>">
			<?php echo getzxfind($row['about'])[0]['title'];?>
			</a>
		</td>
      ...
      <td align="center">
		<?php if( !empty($row["username"]) ){ ?>
			<a href="usermanage.php?keyword=<?php echo $row["username"]?>"><?php echo $row["username"]?></a>
		<?php }else{ ?>
			<a href="usermanage.php?id=<?php echo $row["user_id"]?>"><?php echo getuserfind($row['user_id'])[0]['username'];?></a>
		<?php } ?>
