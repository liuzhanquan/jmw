  ...
  <tr class="trcontent"> 
	...
	<?php 
		...
		if( !empty($dl_sql) ){
			...
			$dzStr = $dzStr.'产品标题：<a href="dl_manage.php?id='.$row['dl_id'].'">'.$dl_sql[0]['cp'].'</a>';
		}
		if( !empty($dl_sql) && !empty($zx_sql) ){
			$dzStr = $dzStr.'<br/>';
		}
		if( !empty($zx_sql) ){
			$dzStr = $dzStr.'资讯标题：<a href="zx_manage.php?id='.$row['zx_id'].'">'.$zx_sql[0]['title'].'</a>';
		}
	?>
