...
$tablenametow=isset($_POST["tablenametow"])?$_POST["tablenametow"]:'';
$id="";
if(!empty($_POST['id'])){
...
while($row=fetch_array($rs)){
	...
	query("delete from `".$tablename."` where id='".$row['id']."'");
	query("delete from `".$tablenametow."` where zid='".$row['id']."'");
...
if (strpos($id,",")>0){
	...
	if($tablename=="zzcms_dl"){
		query("delete from `zzcms_dlmessage` where did in (". $id .")");
		query("delete from `zzcms_ad` where dl_id in (". $id .")");
		query("delete from `zzcms_adlist` where did in (". $id .")");
		query("delete from `zzcms_usercollect` where did in (". $id .")");
	}
}else{
	$sql="delete from `".$tablename."` where id='$id'";
	if($tablename=="zzcms_dl"){
		query("delete from `zzcms_dlmessage` where did='$id'");
		query("delete from `zzcms_ad` where dl_id='$id'");
		query("delete from `zzcms_dllist` where did='$id'");
		query("delete from `zzcms_usercollect` where did='$id'");
