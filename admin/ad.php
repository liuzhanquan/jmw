...
$isok=query("INSERT INTO zzcms_ad (bigclassname,smallclassname,title,titlecolor,link,dl_id,zx_id,img,username,starttime,endtime,elite,sendtime)
VALUES
('$bigclassname','$smallclassname','$title','$titlecolor','$link','$dl_id','$zx_id','$img','$username','$starttime','$endtime','$elite','".date('Y-m-d H:i:s',time()-(showadvdate+1)*60*60*24)."')");
$id=insert_id();
}elseif ($_REQUEST["action"]=="modify") {
checkadminisdo("adv_modify");
$isok=query("update zzcms_ad set bigclassname='$bigclassname',smallclassname='$smallclassname',title='$title',titlecolor='$titlecolor',link='$link',zx_id='$zx_id',dl_id='$dl_id',
...
<option value="<?php echo $row["classname"]?>" <?php if ($row["classname"]==@$_COOKIE["smallclassid"]) { echo "selected";}?>><?php echo $row["classname"]?></option>
	...
	<tr> 
      <td align="right" class="border">关联代理产品(id)：</td>
      <td class="border">
			<input name="dl_id" type="text" id="dl_id" size="50"> 不关联为空或填0
	  </td>
    </tr>
	<tr> 
      <td align="right" class="border">关联文章(id)：</td>
      <td class="border">
			<input name="zx_id" type="text" id="zx_id" size="50"> 不关联为空或填0
	  </td>
    </tr>
    <tr> 
...
<option value="<?php echo $rown["classname"]?>" <?php if ($rown["classname"]==$row["smallclassname"]) { echo "selected";}?>><?php echo $rown["classname"]?></option>
	...
	<tr> 
      <td align="right" class="border">关联代理产品(id)：</td>
      <td class="border">
			<input name="dl_id" type="text" id="dl_id" value="<?php echo $row["dl_id"]?>" size="50"> 不关联为空或填0
	  </td>
    </tr>
	<tr> 
      <td align="right" class="border">关联文章(id)：</td>
      <td class="border">
			<input name="zx_id" type="text" id="zx_id" value="<?php echo $row["zx_id"]?>" size="50"> 不关联为空或填0
	  </td>
    </tr>
    <tr> 
      <td align="right" class="border">标题：</td>
