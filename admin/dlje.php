...
$aa = query("update `".$_COOKIE['tablename']."` set xuhao='$xuhao' where id='".$row['id']."'");
...
<div class="border2">
<input type="submit" class="buttons" onClick="javascript:location.href='?dowhat=addbigclass'" value="添加金额区间">
</div>
<?php
...
<form name="form1" method="post" action="?action=px">
  <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
    <tr class="trtitle"> 
      <td width="10%"  >ID</td>
      <td width="10%" align="center" >最小金额 </td>
      <td width="10%" align="center" >最大金额 </td>
      <td width="15%"  >排序</td>
      <td width="20%"  >操作</td>
    </tr>
    ...
    <tr class="bgcolor1">
      ...
      <td align="center"><?php echo $row["price_min"]?></td >
      <td align="center"><?php echo $row["price_max"]?></td >
	  <?php
...
if ($action=="add"){
	...
	$aa = query("insert into `".$_COOKIE['tablename']."` (price_min,price_max,update_time)values('$price_min','$price_max','".time()."')");
	$bcid=insert_id();
...
?>
<div class="admintitle">添加<?php echo $TitleClass?></div>
<form name="form1" method="post" action="?dowhat=addbigclass">
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    <tr> 
      <td width="100%" class="border">
		<div id="add">
	  <input name="price_min" type="text" id="price_min" size="25" maxlength="50" placeholder="请填写最小金额" > 万 - 
	  <input name="price_max" type="text" id="price_max" size="25" maxlength="50" placeholder="请填写最大金额" > 万
	 <hr/>
	  </div>	  
...
$price_max=trim($_POST["price_max"]);
	...
	query("update `".$_COOKIE['tablename']."` set price_min='$price_min',price_max='$price_max',update_time='".time()."' where id='" .$id."'");
...
$row=fetch_array($rs);
?>
<div class="admintitle">修改<?php echo $TitleClass?></div>
<form name="form1" method="post" action="?dowhat=modifybigclass" onSubmit="return CheckForm();">
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    <tr> 
      <td align="right" class="border">金额区间修改：</td>
      <td class="border"> 
		<input name="id" type="hidden" id="id" value="<?php echo $row["id"]?>" size="30" maxlength="30">
		<input name="price_min" type="text" id="price_min" value="<?php echo $row["price_min"]?>" size="30" maxlength="30">  万 - 
        <input name="price_max" type="text" id="price_max" value="<?php echo $row["price_max"]?>" size="30" maxlength="30"> 万
		</td>
    </tr>
