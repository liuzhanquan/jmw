 ...
 *
 */
function dl_level_sel( $dl_id, $html = 0,$cpname = '' ){
	$res = query_arr('select classid from zzcms_dl where id="'.$dl_id.'"');
	$zsclass = query_arr('select classid,parentid,classname from zzcms_zsclass where classid="'.$res[0]['classid'].'"');
	if( !empty( $cpname ) ){
		$zsclass[0]['classname'] = $cpname;
	}
	if( $html == 0 ){
		return $zsclass;
	}else{
		if( $zsclass[0]['parentid'] == 0 ){
			$str = '<a href="dl_manage.php?b='.$zsclass[0]['classid'].'">'.$zsclass[0]['classname'].'</a>';
		}else{
			$str = '<a href="dl_manage.php?b='.$zsclass[0]['parentid'].'&pid='.$zsclass[0]['classid'].'">'.$zsclass[0]['classname'].'</a>';
		}
			return $str;
	}
 ...
 *
 */
function getdlfind($id){
	$res = query_arr('select cp,classid,id from zzcms_dl where id="'.$id.'"');
 ...
 *
 */
function getuserfind($id){
	$res = query_arr('select id,username,phone from zzcms_user where id="'.$id.'"');
 ...
 *
 */
function getzxfind($id){
	$res = query_arr('select id,title,laiyuan from zzcms_zx where id="'.$id.'"');
 ...
 *
 */
function getadfind($classname,$parentid,$did){
	$res = query('select id from zzcms_ad where smallclassname= "'.$classname.'" and bigclassname="'.$parentid.'" and dl_id ="'.$did.'"');
	$state = num_rows($res);
	return $state;
 ...
 *
 */
function getadstatus($classid,$adclassid){
	$adInfo = query_arr('select classname from zzcms_adclass where classid='.$adclassid);
	$adStatus = query_arr('select classid from zzcms_adclass where parentid="'.$adInfo[0]['classname'].'" and dl_classid="'.$classid.'"');
	
	
	if( empty($adStatus[0]['classid']) ){
		$state = 0;
	}else{
		$state = $adStatus[0]['classid'];
	}
